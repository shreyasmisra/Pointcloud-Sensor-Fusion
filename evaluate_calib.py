...
from DatasetLidarCamera import DatasetLidarCameraKittiOdometry, DatasetLidarCameraKittiRaw
...
def config():
    ...
    output = '../output' + dataset[-4:] + str(RUN)
    save_image = False
...
def main(_config, seed):
    ...
    img_shape = (384, 1280) # change this
    input_size = (256, 512)
    ...
    if _config['save_log']:
        ...
        log_file = f'logs/log_run{_config["RUN"]}_dataset_raw.csv'
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])): # what is sample rgb?
            ...
            depth_show = depth_img.detach().cpu().numpy()
            ...
            if batch_idx == -1:
                ...
                plt.imsave(f"depth_init_kitti.png", depth_img.detach().cpu().numpy()[0])
                plt.imsave(f"rgb_kitti.png", sample['rgb'][idx].detach().cpu().numpy()[0])
                plt.imsave(f"depth_gt_kitti.png", depth_gt.detach().cpu().numpy()[0])
        ...
        # print(lidar_input[lidar_input!=0])
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                #rotated_point_cloud = rotate_forward(sample['point_cloud'][0], RT_predicted) # H_pred*X_init
                #rotated_point_cloud = RT_predicted.cpu() @ sample['point_cloud'][0].cpu() # H_pred*X_init
                ...
                if batch_idx == -1:
	                ...
	                #plt.imsave(f"depth_pred_kitti_{iteration}.png", depth_img_pred.detach().cpu().numpy()[0])
