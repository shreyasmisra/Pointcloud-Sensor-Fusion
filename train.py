...
_config = {
    ...
    'data_folder': '/home/shreyasm/lccnet/LCCNet/KITTI/data_odometry_color/',
...
log_name = f"{_config['dataset'][6:]}_{_config['max_t']}t_{_config['max_r']}r.txt"
_run = Run(log_name, _config)
...
def train(model, optimizer, rgb_img, refl_img, target_transl, target_rot, loss_fn, point_clouds, loss):
    ...
    # target_transl = target_transl[:,:,0].type(torch.FloatTensor).cuda()
    # target_rot = target_rot.type(torch.FloatTensor).cuda()

    if loss == 'points_distance' or loss == 'combined':
        losses = loss_fn(point_clouds, target_transl, target_rot, transl_err, rot_err)
    else:
        losses = loss_fn(target_transl[:,:,0], target_rot, transl_err, rot_err)
...
def main(_config):
    ...
    torch.save(model.state_dict(), model_savepath + f'/lab_sensors_finetune_rot_5_trans_0.5_{total_train_loss / len(dataset_train)}.pth') 
