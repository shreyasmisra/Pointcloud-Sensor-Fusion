...
#        print("before", np.asarray(rgb).shape)
        rgb = to_tensor(rgb)
        #print(rgb[0,:,:].cpu().numpy().mean())
...
#        print("after2", rgb.shape)
    ...
    def __getitem__(self, idx):
        ...
        # pcd1 = o3d.cuda.pybind.geometry.PointCloud()
        # pcd1.points = o3d.cuda.pybind.utility.Vector3dVector(pc[:,:3])
        ...
        # pcd2 = o3d.cuda.pybind.geometry.PointCloud()
        # pcd2.points = o3d.cuda.pybind.utility.Vector3dVector(pc_rot[:3,:].T)
        ...
        R = mathutils.Euler((rotx, roty, rotz), 'XYZ') # camera pose - rotation
        T = mathutils.Vector((transl_x, transl_y, transl_z)) # camera pose - translation
        # print("1", R, T)
        # print("2 init", initial_RT)
        R, T = invert_pose(R, T)
        ...
        if self.split == 'test':
            ...
            # print("pc", sample['point_cloud']) # 4 x m
            # print("cl", sample['calib'])
            # print("3 extrin", sample['extrin']) # ground truth
            # print("4 r", sample['rot_error'])
            # print('5 t', sample['tr_error'])
...
class DatasetLidarCameraKittiRaw(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, use_reflectance=False,
                 max_t=1.5, max_r=15.0, split='val', device='cpu', val_sequence='2011_09_30_drive_0028_sync'):
        ...
        if split == 'val' or split == 'test':
            val_RT_file = os.path.join(dataset_dir, 
                                       f'val_RT_seq{date}_{max_r:.2f}_{max_t:.2f}.csv')
            if os.path.exists(val_RT_file):
                print(f'VAL SET: Using this file: {val_RT_file}')
                df_test_RT = pd.read_csv(val_RT_file, sep=',')
                # print(df_test_RT)
                ...
                for i in range(len(self.all_files)):
                    ...
                    self.val_RT.append([float(i), transl_x, transl_y, transl_z,
                                         rotx, roty, rotz])
            print("sd kjlh", len(self.all_files), len(self.val_RT))

            # new code
            if len(self.val_RT) > len(self.all_files):
                self.val_RT = self.val_RT[:len(self.all_files)]
    ...
    def __getitem__(self, idx):
        ...
        # print("pc", sample['point_cloud']) # 4 x m
        # print("cl", sample['calib'])
        # print("init", sample['initial_RT'])
        # print("extrin", sample['extrin']) # ground truth
...
if __name__ == '__main__':
    ...
    dataset = 'kitti/raw'
    data_folder = 'KITTI/data_odometry_color'
    test_sequence = 0
    ...
    dataset_class = DatasetLidarCameraKittiOdometry(data_folder)
    ...
    TestImgLoader = torch.utils.data.DataLoader(dataset=dataset_class,
                                                ...
                                                pin_memory=True)
    #print(dataset_class)
    sample = next(iter(DatasetLidarCameraKittiOdometry(data_folder)))

    print(sample['rgb'].shape)
    #print(sample['point_cloud'].detach().cpu().numpy().max(axis=1))
    #print(sample['calib'])
    #print(sample['extrin'])
